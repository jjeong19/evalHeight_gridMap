...
parser.add_argument('--objHeight', type=str)
parser.add_argument('--objD', type=str)
args = parser.parse_args()


RMSE_sum = np.zeros(args.numObj)
heightList = args.objHeight.split(",")
heightList = [int(x) for x in heightList]
obj_height = np.array(heightList)

distanceList = args.objD.split(",")
distanceList = [float(x) for x in distanceList]
obj_distance = np.array(distanceList)
...
#     print(height)
    ...
    for idxObj in range(args.numObj):
        ...
        target_array = np.reshape(height, (200, 200))[c_obj[idxObj], r_obj[idxObj]]*100
        # print(target_array)
        target_bool = np.invert(np.isnan(target_array))
        target_array = target_array[target_bool]
        c = c_obj[idxObj][target_bool]
        r = r_obj[idxObj][target_bool]
        # print(target_array)
        target_array_bool = (target_array < obj_height[idxObj] + 10) * (target_array > obj_height[idxObj] - 10)
        target_array = target_array[target_array_bool]
        r = r[target_array_bool]
        c = c[target_array_bool]
    #     print(np.round(target_array))
        
        if obj_distance[0] == 0:
            # pdb.set_trace()
            RMSE = np.sqrt(np.sum(((target_array-obj_height[idxObj])/ (0.02*np.sqrt((c-100)**2 + (r-100)**2)))**2)/np.count_nonzero(target_array))
